...
- 对外推理仅提供 OpenAI `POST /v1/responses`，当前范围为非流式、无服务端会话存储的文本请求；Chat Completions 仅作为内部 MLX 传输，Anthropic 与公开 Chat 路由均返回 404。流式、工具调用、图片、音频、文件、服务端会话和未实现参数必须明确报错，不能静默丢弃。模型能力列表必须反映网关实际开放能力。
